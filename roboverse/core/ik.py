...
def ik(body, link, pos, theta, damping):
    ...
    ik_solution = p.calculateInverseKinematics(body, link, pos, targetOrientation=theta, jointDamping=damping)
...
def velocity_control(body, joints, velocities):
    ...
    p.setJointMotorControlArray(body, joints, p.VELOCITY_CONTROL, targetVelocities=velocities)
...
def sawyer_ik(body, link, pos, theta, gripper, damping=1e-3, gripper_close_thresh=0.5, gripper_vel_mult=20):
    #### get gripper state position
    l_limits = get_joint_info(body, 'right_gripper_l_finger_joint', ['low', 'high'])
    r_limits = get_joint_info(body, 'right_gripper_r_finger_joint', ['low', 'high'])
