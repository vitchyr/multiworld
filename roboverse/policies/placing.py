...
class PlacingPolicy:
    ...
    def get_action(self, obs):
        ...
        ee_z = ee_pos[-1]
        goal_z = self._goal_pos[-1]
        obj_goal_dist = np.sum((obj_pos - self._goal_pos)**2)
        ...
        if (self._gripper == self._gripper_open and max_delta < self._gripper_close_delta) or \
           (self._gripper == self._gripper_close and max_delta < self._gripper_remain_closed_delta):
            ...
            if ee_z < goal_z - 0.2:
                ## if gripper low to ground, first lift up
                goal_pos[:2] = ee_pos[:2]
            delta_pos = np.clip((goal_pos - ee_pos), -1, 1)
        else:
            self._gripper = self._gripper_open
            delta_pos = np.clip(delta_pos * 10, -1, 1)

        if obj_goal_dist < 0.005:
            self._gripper = self._gripper_open
